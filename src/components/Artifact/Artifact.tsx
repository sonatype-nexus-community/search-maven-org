...
import { NxCard, NxH3, NxH4, NxTextInput, NxTextLink, NxGrid, NxList } from '@sonatype/react-shared-components';
...
const Artifact = () => {
  ...
  useEffect(() => {
    artifactContext.fetchRemoteContent(remoteRepositoryPomLink()).then(value => {
      if (value) {
    ...
    artifactContext.fetchRemoteContent(remoteRepositoryJarSha1Link()).then(value => {
      // some sha1 files have path names in them after a space, this way we remove the path part.
          ...
          <NxCard.Content>
                      ...
                      pomParsed?.licenses.map((license, index
                        ...
                        return (
                          <NxList.Item key={index}>
                            <NxTextLink href={license.url} target="_blank">{license.name}</NxTextLink>
                          </NxList.Item>
                        );
                      })
                    }
...

            {
              ...
              <NxGrid.Row>
                ...
                <NxGrid.Column>
                  <NxTextLink href={pomParsed.url} target="_blank">{pomParsed.url}</NxTextLink>
                </NxGrid.Column>
              </NxGrid.Row>
            }

            {
              ...
              <NxGrid.Row>
                ...
                <NxGrid.Column>
                  <NxTextLink href={pomParsed.scmUrl} target="_blank">{pomParsed.scmUrl}</NxTextLink>
                </NxGrid.Column>
              </NxGrid.Row>
            }

            {
              ...
              pomParsed?.organizationUrl &&
              <NxGrid.Row>
                <NxGrid.Column>
                  Organization
                </NxGrid.Column>
                <NxGrid.Column>
                  <NxTextLink href={pomParsed.organizationUrl}
                              target="_blank">{pomParsed.organizationName}</NxTextLink>
                </NxGrid.Column>
              </NxGrid.Row>
            }

            {
              pomParsed?.developers &&
              pomParsed?.developers?.length !== 0 &&
              <NxGrid.Row>
                <NxGrid.Column>
                  Developers
                </NxGrid.Column>
                      ...
                      pomParsed?.developers.map((developer, index
                        ...
                        return (
                          <NxList.Item key={index}>
                            ...
                            &nbsp;
                            <NxTextLink href={`mailto:${developer.email}`}>({developer.email})</NxTextLink>

                            {
                              ...
                              <>
                                , <NxTextLink
                                  href={developer.organizationUrl}>{developer.organization}</NxTextLink>
                              </>
                            }
                          </NxList.Item>
                        );
                  ...
                  <NxList>
                    {
                      pomParsed?.mailingLists.map((mailingList, index
                        ...
                        return (
                          <NxList.Item key={index}>
                            <NxTextLink
                              href={mailingList.archiveUrl}>{mailingList.name}
                            </NxTextLink>
                          </NxList.Item>
                        );
                      })
...

            {
              ...
              <NxGrid.Row>
                ...
                <NxGrid.Column>
                  ...
                  <NxTextLink href={`/artifact/${pomParsed.relocationGroupId}/${pomParsed.relocationArtifactId}`}>
